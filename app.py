...
with st.sidebar:
    ...
    if sessions:
        for s in sessions:
            risk_icon = {"low": "🟢", "med": "🟡", "high": "🔴"}.get(s["worst_risk"], "⚪")
            label = f"{risk_icon} {s['pr_url'].split('/')[-1]}  ({s['events']} events)"
            if st.button(label, key=s["thread_id"], help=s["thread_id"]):
        ...
        st.caption("No sessions yet — run a review first.")
